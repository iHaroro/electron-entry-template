  ...
  <div class="single-device-box">
    <div class="single-device-title">
      <div class="device-info-tab">
        <div
          v-for="(item, index) in systemProcessTabs"
          class="device-info-tab_item"
          :key="`device-info-tab-item_${index}`"
          :class="{ active: mainDeviceType === index }"
          @click="changeMainDeviceType(index)"
        >
          {{ item.name }}
        </div>
      </div>
    ...
    <div class="single-device-info">
      ...
      <div class="single-device-info-left">
        <template v-if="processFields?.length">
          <template v-for="fieldKey in processFields" :key="`single-process-line-${fieldKey}`">
            <div
...
// 当前主机设备类型
const mainDeviceType = ref(0)

/**
 * @function changeMainDeviceType
 * @description 改变设备类型
 * @param {number} index 下标值
 **/
const changeMainDeviceType = (index) => {
  mainDeviceType.value = index
}
...
/**
 * @function deviceParamData
 * @description 设备参数数据
 * @returns {object} 设备参数数据
 **/
...
/**
 * @function systemProcessTabs
 * @description 设备参数Tab数据
 * @returns {array} 设备参数Tab数据
 **/
const systemProcessTabs = computed(() => {
  return props.deviceInfo.config?.system || []
})

/**
 * @function processFields
 * @description 参数分类字段列表
 * @returns {array} 参数分类字段列表
 **/
const processFields = computed(() => {
  console.log(systemProcessTabs.value[mainDeviceType.value])
  return systemProcessTabs.value[mainDeviceType.value]?.processFields.filter((key) => {
