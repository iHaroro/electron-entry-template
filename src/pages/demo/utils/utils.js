 ...
 **/
export const getDataByValue = (dataObject, value, options = { modelValue: 'value' }) => {
...
/**
 * @function getArrayMapFromMap
 * @description 将Map转为数组
 * @param {object} map map对象
 * @returns {array} 返回数组
 **/
export const getArrayMapFromMap = (map) => {
  const fields = Object.keys(map)
...
/**
 * @function getRandom
 * @description 获取区间随机数（整数）
 * @param {number} min 最小值（整数）
 * @param {number} max 最大值（整数）
 * @returns {number} 返回区间随机数（整数）
 **/
...
/**
 * @function getReFillFile
 * @description 获取回填的上传的文件对象
 * @param {object} data 上传的文件对象
 * @returns {object} 返回回填的上传的文件对象
 **/
export const getReFillFile = (data) => {
  return {
    uid: data.id,
    name: data.fileName,
    status: 'done',
 ...
 * @param {Array<string>} newValues 新数组
 * @param {Array<string>} oldValues 旧数组
 * @returns {{addedValues: Array<string>, removedValues: Array<string>}} 返回包含新增和移除的数组
 */
